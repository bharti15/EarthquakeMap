...
public class EarthquakeCityMap extends PApplet {
	...
	public void setup() {
		...
		for (Marker mk: markers){
			if ((float)mk.getProperty("depth")<4.0){
				mk.setColor(blue);
//				mk.setStrokeWeight(2);
			}
			else if((float)mk.getProperty("depth")>=4.0 && (float)mk.getProperty("depth")<=4.9) {
				mk.setColor(yellow);
//				mk.setStrokeWeight(3);
			}
			else if((float)mk.getProperty("depth")>=5.0){
				mk.setColor(red);
//				mk.setStrokeWeight(4);
			}
			else{
				mk.setColor(gray);
			}
